    ...
    const grabarCambios = async () => {
        try {
            if (datos.id == -1) {
                ...
                datos.cantidad_up = 0;
                let resultado = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/proyectos/`, datos);
                console.log(resultado);
                alert("Proyecto cargado con éxito");
            } else {
                let resultado = await axios.put(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/proyectos/${datos.id}`, datos);
                console.log(resultado);
